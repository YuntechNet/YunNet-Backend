...
class Permission(SQLBase):
    def check_permission(self, username, code) -> bool:
        ...
        sql =  \
            ...
            SELECT * 
            FROM user 
            INNER JOIN `group` ON user.group_id = `group`.code OR user.extend_group LIKE CONCAT('%',`group`.code,'%')    
            WHERE user.account_id = %username
            AND `group`.per LIKE CONCAT('%',%code,'%')
	        OR user.extend_per LIKE CONCAT('%',%code,'%')
	        AND NOT user.exclude_per LIKE CONCAT('%',%code,'%')"""
